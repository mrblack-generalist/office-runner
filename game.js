...
function create() {
  ...
  for (let x = 0; x <= config.width; x += 130) {
    const t = ground.create(x, 670, 'floor').setOrigin(0, 0);
    t.refreshBody(); floorTiles.push(t);
  }
  ...
  scoreText = this.add.text(20,20,'Score: 0',{ fontFamily:'Arial', fontSize:'32px', fontStyle:'italic', fontWeight:'bold', color:'#000', stroke:'#fff', strokeThickness:2 }).setDepth(11);
  timerText = this.add.text(20,60,'Time: 00:00:000',{ fontFamily:'Arial', fontSize:'32px', fontStyle:'italic', fontWeight:'bold', color:'#000', stroke:'#fff', strokeThickness:2 }).setDepth(11);

  // Lives
  for(let i=0;i<3;i++){
    const h = this.add.image(config.width-40 - i*40,30,'heart').setScale(0.05).setDepth(11);
    heartIcons.push(h);
  }
  ...
  // Level text
  levelText = this.add.text(config.width-140,70,'Level: 1',{ fontFamily:'Verdana', fontSize:'25px', fontWeight:'bold', color:'#000' }).setDepth(11);

  // Start Screen (opaque overlay)
  const cx = config.width/2, cy = config.height/2;
  const overlay = this.add.rectangle(cx,cy,config.width,config.height,0x000,1).setDepth(19);
  const s1 = this.add.text(cx,cy-60,'Your boss is chasing you over quarterly sheet mishaps!',{ fontSize:'24px', fill:'#fff' }).setOrigin(0.5).setDepth(20);
  const s2 = this.add.text(cx,cy-20,'←→ move, ↑ jump, ↓ drop. Avoid printers/desks; coffee = +1 life & +100 pts.',{ fontSize:'18px', fill:'#fff' }).setOrigin(0.5).setDepth(20);
  const s3 = this.add.text(cx,cy+40,'Press SPACE to Start',{ fontSize:'20px', fill:'#0f0' }).setOrigin(0.5).setDepth(20);
  startScreen = this.add.container(0,0,[overlay,s1,s2,s3]).setVisible(true);

  // End Screen
  const eOv = this.add.rectangle(cx,cy,config.width,config.height,0x000,1).setDepth(19);
  const eMsg = this.add.text(cx,cy-40,'',{ fontSize:'32px', fill:'#fff' }).setOrigin(0.5).setDepth(20);
  const e2 = this.add.text(cx,cy+20,'SPACE = Restart | ESC = Home',{ fontSize:'20px', fill:'#0f0' }).setOrigin(0.5).setDepth(20);
  endScreen = this.add.container(0,0,[eOv,eMsg,e2]).setVisible(false);
  endScreen.msgText = eMsg;

    // Score timer (paused until game start)
...
function update() {
  ...
  if(cursors.up.isDown && player.body.touching.down) player.setVelocityY(-470);
  ...
  if(sec%20===0 && sec!==0 && sec!==this.lastBoost) { gameSpeed+=2; obstacleSpeed-=50; this.lastBoost=sec; }

  // Level & BG
  const lvl = Math.floor(sec/60)+1;
  if(lvl!==level) {
    level=lvl; levelText.setText(`Level: ${lvl}`);
...
function spawnCoffee(scene,x){
  ...
  coffees.push(c); scene.physics.add.collider(c,ground);
  scene.physics.add.overlap(player,c,()=>handleCoffeePickup(scene,c),null,scene);
}

function handleHit(scene){
  if(gameOver) return; const now = this.time.now;
  if(now-lastHitTime<HIT_COOLDOWN) return; lastHitTime=now;
  lives--; if(lives>=0) heartIcons[lives].setVisible(false);
  this.tweens.add({targets:player,alpha:0,yoyo:true,repeat:5,duration:200});
  hitPopup.setAlpha(1); hitPopup.setY(config.height/2-50);
...
function handleCoffeePickup(scene,coffee){
  coffee.destroy(); score+=100; scoreText.setText(`Score: ${score}`);
  rewardPopup.setText('+100'); rewardPopup.setAlpha(1); rewardPopup.setY(config.height/2-100);
  scene.tweens.add({targets:rewardPopup,alpha:0,y:'-=40',duration:800});
  if(lives<3){ heartIcons[lives].setVisible(true); lives++;
    const hb=scene.add.text(config.width/2,config.height/2-120,'+1 ❤️',{fontSize:'40px',fill:'#f36',fontStyle:'bold'})
      .setOrigin(0.5);
    scene.tweens.add({targets:hb,alpha:0,y:'-=30',duration:1000,onComplete:()=>hb.destroy()});
  }
}
